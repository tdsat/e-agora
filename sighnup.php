...
if(!isset($_SESSION['idUsers'])){
	//Initialize values
    $failedSignUp = TRUE; //Because when we open the page we haven't tried to sign up
    $sighnupbutton=NULL; // We haven't yet clicked the sign up button
    ...
    //We check if the sign up button has been pressed
    if($sighnupbutton)
    {
		//In $successfullSignUp we store the result of the call. If it's 0, then the sign up was succesful
        $failedSignUp=user_create($username,$password,$email,$fName,$lName,$address,$pcode,$city,$country,$phone,$dob);
        if ($failedSignUp){
			//If the sign up was not successful, we show the error messages
		    echo "<div class='error'>The following errors happend while trying to send your request</br>";
        ...
        else //Else we display the successful sign up message
			echo "<h1>Sign up was successful</h1>";
    }
	
//If there was no sign up or the sign up was not successful, we show the form
if($failedSignUp){ ?>
	<h1> Register new user</h1>
		<hr />
		<form method="POST" action="index.php?action=sighnup">
		   <h3> Create eAgora user id and password...These fields are required</h3>
	<table>
		<tr>
			<td>Create eAgora user ID</td>
	...
	By clicking sign up I agree that :
		...
		<ul>
		<li>I've read the terms and conditions</li>
		<li>I'm over 18 years old</li>
		</ul>

			<input type="submit" name="sighnupbutton" value="Sign up"/>
	</form>
<?php }
}
else echo "<h2>You are already registered<h2>"
?>
